...
def make_naive_exploration_scheduler(
    ...
    conv_accuracy = (
        config["conv_accuracy"] if old_style else config["explore"]["conv_accuracy"]
    )
    max_numb_iter = (
    ...
    for job_ in model_devi_jobs:
        ...
        trust_level = TrustLevel(
            ...
            if old_style
            else config["explore"]["f_trust_hi"],
            level_v_lo=config.get("model_devi_v_trust_lo")
            if old_style
            else config["explore"]["v_trust_lo"],
            level_v_hi=config.get("model_devi_v_trust_hi")
            if old_style
